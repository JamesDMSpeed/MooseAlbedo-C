...
#Data Filtering ---------
        ...
        #KMEANS data -----------
                ...
                #CSV w/ normalized data
                k_norm <- read.csv("../../Approach_1_SatSkog/Output/Attempt_2_Visualization/Initial_Plots/kmeans_clustering/kmeans_normalized_variables.csv", header = T)

                #Shapefile
                k_shp <- st_read("../../Approach_1_SatSkog/Output/Attempt_2_Visualization/Initial_Plots/kmeans_clustering/clusters/shapefile/kmeans_clustered.shp")
                

        #CLARA data ------------
                
                #CSV w/ normalized data
                c_norm <- read.csv("../../Approach_1_SatSkog/Output/Attempt_2_Visualization/Initial_Plots/clara_clustering/delimited/clara_normalized_variables.csv", header = T)
                
                #Shapefile
                c_shp <- st_read("../../Approach_1_SatSkog/Output/Attempt_2_Visualization/Initial_Plots/clara_clustering/delimited/clusters/shapefile/clara_clustered.shp")
                
        #Centroids (for plot w/ line) -------------
        centroids <- st_centroid(st_geometry(k_norm), of_largest_polygon = TRUE)
        
```

# Overview
This report contains an overview of "Clustering Attempt #1" for the SatSkog Approach. As discussed in our last meeting, there was too much variation in the Albedo vs. Moose Density plots to see meaningful trends (these plots included data for all of Norway). It was suggested that I try to **cluster** the data or identify some homogenous areas where elevation, SWE, or temperature might be similar - by doing this, it would be possible to reduce variation in albedo caused by SWE/temperature and actually see trends between albedo and moose density.

To do this, I've tried to use both **K-means** and **K-medoids** approaches to cluster the data into somewhat homogenous areas (i.e. regionalize the data). I would love suggestions on how I can better cluster, or if there are some alternative approaches I should try!
...
From the large albedo/SatSkog dataset I produced, I filtered to data from 1999 only - I thought that this would make analysis simpler and prevent any confounding effects that including data from other years might produce. The final geometry of this plot can be seen below:

```{r echo = FALSE, eval= TRUE, message = FALSE}

                #Plot geometry of 1999 data
                ggplot() +
                      geom_sf(data = data_viz, lwd = 0.1)
                
```

I decided to "cut off" the plots in northern Norway (above 7250000 UTM33N) and focus on central/southern Norway - I did this mainly due to the fact that including northern Norway in cluster analysis made clusters much less homogenous. The line I used to delimit the data can be seen below:

```{r echo = FALSE, eval= TRUE, message = FALSE}

                #Plot centroids w/ abline
                plot(centroids, main = "Delimited forest plots - line at 7250000 UTM33N")
                
                #Add a line to "bisect" northern and southern data
                abline(a = 7250000, b = 0)
...
#Selecting the Data
I chose to cluster the data with the following variables (per our last meeting):

* Elevation (average plot elevation based on digital elevation model)
* SWE (Months 1-12 in 1999)
* Temperature (Months 1-12 in 1999)

***

#Standardizing the Data
Before clustering using either K-means or K-medoids, I standardized the data that would be used for clustering:

```{r echo = TRUE, eval= FALSE, message = FALSE}

        #Standardize the data (using caret package) & remove NA rows ----------
        preproc <- preProcess(data, method=c("center", "scale"))
        std <- predict(preproc, data) #Data standardized
