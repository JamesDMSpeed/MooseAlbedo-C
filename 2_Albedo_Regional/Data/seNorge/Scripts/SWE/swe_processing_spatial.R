...
#DATA PROCESSING ------------------------------------------------------------------------------
        ...
        files <- list.files(path="/Volumes/JS_Ext_HD/THESIS/SeNorge_Data/SWE", pattern="*.nc", full.names=TRUE, recursive=FALSE)
        
        #Loop through and process all seNorge netCDF SWE files
        for(file in files){
                ...
                #DATA IMPORT + FORMATTING -----------
                        ...
                        filename <- paste("/Volumes/JS_Ext_HD/THESIS/SeNorge_Data/SWE/", filename, '.nc', sep = '')
                        
                        #Load in gridded SWE data for 1999 into 3D RasterBrick object
                        data <- brick(filename, varname = "snow_water_equivalent")
                ...
                writeRaster(swe_means,
                            filename = paste("/Volumes/JS_Ext_HD/THESIS/SeNorge_Output/SWE/swe_means_", year, "_all_norway.tif", sep = ''),
                            format = "GTiff",
                            overwrite = T)
