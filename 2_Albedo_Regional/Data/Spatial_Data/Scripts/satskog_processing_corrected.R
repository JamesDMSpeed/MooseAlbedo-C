...
#PROCESS DATA ------------------  
        ...
        for(dir in dirs){
                ...
                #Define output path
                output_path <- paste("2_Albedo_Regional/Data/Spatial_Data/Output_Corrected/",
                                     dir_base,
                                     '/',
                                     sep = '')
                                                        ...
                                                        #Calculate albedo for each polygon ------
                                                                ...
                                                                ## NOTE: Doing this in a vectorized manner - GREATLY REDUCES TIME
                                                                        ...
                                                                        for(j in 1:12){
                                                                                ...
                                                                                #Spruce albedo
                                                                                satskog_sub[, paste("Month_", j, "_Albedo_Spruce", sep = "")] <- 0.077+0.072*(1-1/(1+exp(-2.354*((satskog_sub[, paste("Temp_Month_", j, sep = "")][[1]])-273.433))))+0.074*(1/(1+exp(-0.191*((satskog_sub[, paste("SWE_Month_", j, sep = "")][[1]])-33.093))))+0.252*exp(-0.023*(satskog_sub$vuprha))*(1-0.7*exp(-0.011*(satskog_sub[, paste("SWE_Month_", j, sep = "")][[1]])))
                                                                                
                                                                                #Pine albedo
                                                                                satskog_sub[, paste("Month_", j, "_Albedo_Pine", sep = "")] <- 0.069+0.084*(1-1/(1+exp(-1.965*((satskog_sub[, paste("Temp_Month_", j, sep = "")][[1]])-273.519))))+0.106*(1/(1+exp(-0.134*((satskog_sub[, paste("SWE_Month_", j, sep = "")][[1]])-30.125))))+0.251*exp(-0.016*(satskog_sub$vuprha))*(1-0.7*exp(-0.008*(satskog_sub[, paste("SWE_Month_", j, sep = "")][[1]])))
                                                                                
                                                                                #Birch albedo
                                                                                satskog_sub[, paste("Month_", j, "_Albedo_Birch", sep = "")] <- 0.085+0.089*(1-1/(1+exp(-2.414*((satskog_sub[, paste("Temp_Month_", j, sep = "")][[1]])-273.393))))+0.169*(1/(1+exp(-0.107*((satskog_sub[, paste("SWE_Month_", j, sep = "")][[1]])-37.672))))+0.245*exp(-0.023*(satskog_sub$vuprha))*(1-0.7*exp(-0.004*(satskog_sub[, paste("SWE_Month_", j, sep = "")][[1]])))
...
#SHAPEFILE UNIFICATION -------------------------------------------------------------------------
                ...
                dirs <- list.dirs(path="2_Albedo_Regional/Data/Spatial_Data/Output_Corrected", full.names=TRUE, recursive=FALSE)
...
#WRITE OUTPUT ----------------------------------------------------------------------------
        ...
        st_write(final_shp, "2_Albedo_Regional/Data/Final_Shapefile/Output/attempt_3/all_data/corrected_shapefile.shp", driver = "ESRI Shapefile", append = FALSE)

        #Write 1999-only ESRI shapefile to relevant output directory
        st_write(final_shp, "2_Albedo_Regional/Data/Final_Shapefile/Output/attempt_3/1999_only/corrected_shapefile_1999.shp", driver = "ESRI Shapefile", append = FALSE)
