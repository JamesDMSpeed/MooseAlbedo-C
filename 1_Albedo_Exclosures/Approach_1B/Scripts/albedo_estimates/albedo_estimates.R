...
#INITIAL DATA IMPORT ----------------------------------------------------------------------------------------------
        ...
        #SUBPLOT STAND VOLUMES (m3/ha) (Trondelag/Hedmark/Telemark: 2009-2019) - circular subplots within each plot/LocalityCode
                ...
                #Load plot volumes
                plot_volumes <- read.csv('1_Albedo_Exclosures/Approach_1B/Output/tree_volumes/tree_volumes.csv', header = TRUE)
        
        
        #SENORGE SWE and TEMP (2001-2019)    
        ...
        ## SUSTHERB TREE SPECIES PROPORTIONS (SUBPLOT LEVEL - 2009-2019)
                ...
                tree_data <- read.csv('1_Albedo_Exclosures/Universal/Output/Tree_Species_Proportions/tree_species_proportions_subplot_level.csv', header = TRUE)
...
#CALCULATE ALBEDO --------------------------------------------------------------------------------------
        ...
        #Calculate albedo for each species within each subplot (across all years of data - 2009-2019)
                ...
                #For each row, add climate data (SWE/Temp)
                        ...
                        for(i in 1:nrow(albedo)){
                                ...
                                #Get climate data for corresponding month and year
                                
                                        #If exclosure, get LocalityCode for corresponding open plot
                                        if(tr == "UB"){
                                                
                                                #Get LocalityCode for browsed plot
                                                c <- as.character(site_data$LocalityCode[site_data$LocalityName == b & site_data$Treatment == 'open'])
                                                
                                                #Temps
                                                albedo[i, "Temp_K"] <- temps$Avg_Temps_K[temps$LocalityCode == c & temps$Year == yr_curr & temps$Month == mt]
                                                
                                                #SWE
                                                albedo[i, "SWE_mm"] <- swe$SWE_mm[swe$LocalityCode == c & swe$Year == yr_curr & temps$Month == mt]
                                                
                                        } else if (tr == "B"){
                                                
                                                #Temps
                                                albedo[i, "Temp_K"] <- temps$Avg_Temps_K[temps$LocalityCode == a & temps$Year == yr_curr & temps$Month == mt]
                                                
                                                #SWE
                                                albedo[i, "SWE_mm"] <- swe$SWE_mm[swe$LocalityCode == a & swe$Year == yr_curr & temps$Month == mt]
        ...
        write.csv(final_albedo, "1_Albedo_Exclosures/Approach_1B/Output/albedo_estimates/albedo_estimates.csv")
...
#CALCULATE MEAN ALBEDO -------------------------------------------------------------------------------------
        ...
        #Aggregate means for each group, month, and year
        albedo_means <- aggregate(final_albedo$Albedo, by = list(final_albedo$Treatment, final_albedo$Year, final_albedo$Group, final_albedo$Month), FUN = mean)
        colnames(albedo_means) <- c("Treatment", "Year", "Group", "Month", "Average_Albedo")
        ...
        #Calculate standard error for each mean
                ...
                for(i in min(albedo_means$Year):max(albedo_means$Year)){
                        ...
                        for(j in 1:12){
                                ...
                                #Birch albedo SE
                                        ...
                                        #Browsed
                                        albedo_means$SE[albedo_means$Group == "Birch" & albedo_means$Year == i & albedo_means$Treatment == "B" & albedo_means$Month == j] <- std(final_albedo$Albedo[final_albedo$Year == i & final_albedo$Group == "Birch" & final_albedo$Treatment == "B" & final_albedo$Month == j])
                                        
                                        #Unbrowsed
                                        albedo_means$SE[albedo_means$Group == "Birch" & albedo_means$Year == i & albedo_means$Treatment == "UB" & albedo_means$Month == j] <- std(final_albedo$Albedo[final_albedo$Year == i & final_albedo$Group == "Birch" & final_albedo$Treatment == "UB" & final_albedo$Month == j])
                                        
                                
                                #Spruce albedo SE
                                
                                        #Browsed
                                        albedo_means$SE[albedo_means$Group == "Spruce" & albedo_means$Year == i & albedo_means$Treatment == "B" & albedo_means$Month == j] <- std(final_albedo$Albedo[final_albedo$Year == i & final_albedo$Group == "Spruce" & final_albedo$Treatment == "B" & final_albedo$Month == j])
                                        
                                        #Unbrowsed
                                        albedo_means$SE[albedo_means$Group == "Spruce" & albedo_means$Year == i & albedo_means$Treatment == "UB" & albedo_means$Month == j] <- std(final_albedo$Albedo[final_albedo$Year == i & final_albedo$Group == "Spruce" & final_albedo$Treatment == "UB" & final_albedo$Month == j])
                                        
                                        
                                #Pine albedo SE
                                
                                        #Browsed
                                        albedo_means$SE[albedo_means$Group == "Pine" & albedo_means$Year == i & albedo_means$Treatment == "B" & albedo_means$Month == j] <- std(final_albedo$Albedo[final_albedo$Year == i & final_albedo$Group == "Pine" & final_albedo$Treatment == "B" & final_albedo$Month == j])
                                        
                                        #Unbrowsed
                                        albedo_means$SE[albedo_means$Group == "Pine" & albedo_means$Year == i & albedo_means$Treatment == "UB" & albedo_means$Month == j] <- std(final_albedo$Albedo[final_albedo$Year == i & final_albedo$Group == "Pine" & final_albedo$Treatment == "UB" & final_albedo$Month == j])

                                        
                                #Composite albedo SE
                                        
                                        #Browsed
                                        albedo_means$SE[albedo_means$Group == "Composite" & albedo_means$Year == i & albedo_means$Treatment == "B" & albedo_means$Month == j] <- std(final_albedo$Albedo[final_albedo$Year == i & final_albedo$Group == "Composite" & final_albedo$Treatment == "B" & final_albedo$Month == j])
                                        
                                        #Unbrowsed
                                        albedo_means$SE[albedo_means$Group == "Composite" & albedo_means$Year == i & albedo_means$Treatment == "UB" & albedo_means$Month == j] <- std(final_albedo$Albedo[final_albedo$Year == i & final_albedo$Group == "Composite" & final_albedo$Treatment == "UB" & final_albedo$Month == j])
...
#NOTE: SCATTERPLOTS ARE SUPER VARIABLE AND NOT VERY USEFUL FOR IDENTIFYING TRENDS - GO WITH MEANS + SE BY TREATMENT
                ...
                #Birch Means
                png(filename = "1_Albedo_Exclosures/Approach_1B/Output/albedo_estimates/means/albedo_estimates_birch.png",
                    width = 1600,
                    height = 1200,
                    bg = "white")
                
                ggplot(subset(albedo_means, Group == "Birch"), aes(x = Year, y = Average_Albedo, color = Treatment, group = Treatment)) +
                        ...
                        labs(x = "Year", y = "Mean Deciduous Albedo") +
