...
while ( i <= max(senorge_temps$trondelag_) ){
        ...
        if( length(lidarYear) > 0 ){
                ...
                temp_frame <- data.frame(senorge_temps[senorge_temps$trondelag_ == i & senorge_temps$X_Year == 2016,])
...
#WRITE OUTPUT --------------------------------------------------
        ...
        write.csv(avg_temps, file = '1_Albedo_Exclosures/Universal/Output/2016_Climate_Data/monthly_avg_temp_2016.csv', row.names = TRUE)
